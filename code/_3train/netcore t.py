...
class cnn_lstm(nn.Module):
    ...
    def __init__(self, params, num_classes):
        ...
        self.binNum4spectrum = round(params.wholeBand.getBandWidth() / params.binWidth4freqHisto)#22
        #--------------------
        # below, an ad-hoc setup of time-bin num for STFT. It would be better if the constant "128" can be removed.
        

        self.stft_time_bin_num = np.int(np.ceil(np.float(1.0 * params.windowSizeInSec * (1.0 * params.samplingFreq / 128) / params.stft_time_bin_in_seconds))) + 1   # plus one from edges = 5
        ...
        self.skip_by = params.torch_skip_by #4
        self.layerNum = len(self.filter_nums)#8
        ...
        if not params.useRawData:
            ...
            if params.useFreqHisto:
                ...
                if params.useTime:
                    ...
                    combined = cat((rawDataPart.view(rawDataPart.size(0), -1), freqFeature.view(freqFeature.size(0), -1)), dim=1)#combine
            elif params.useTime:
